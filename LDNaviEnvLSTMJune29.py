...
DATA_LIST = [\
             # '/home/ryan/data/navi/ewap_dataset_full/seq_eth/', \
             '/home/ryan/data/navi/ewap_dataset_full/seq_hotel/', \
             '/home/ryan/data/navi/socially compliant/',\
             '/home/ryan/data/navi/zara01/', \
             '/home/ryan/data/navi/zara02/', \
             '/home/ryan/data/navi/zara03/', \
             '/home/ryan/data/navi/ucy03/', \
             '/home/ryan/data/navi/ucy01/'
]

# DATA_LIST = ['/home/ryan/data/navi/ewap_dataset_full/seq_eth/', \
#              '/home/ryan/data/navi/ewap_dataset_full/seq_hotel/', \
#              '/home/ryan/data/navi/socially compliant/']
...
GOAL_SWITCHING_BOUND=[1.2,MAX_COM_DIST]
...
class LDNaviEnvLSTMJune29(Env):
    ...
    def state_space(self):
        space_low = np.ones(STATE_DIM*STACK_STATE_FRAMES,)*(-BOUND_OF_SPACE)
        for i in xrange(0,STACK_STATE_FRAMES):
            space_low[1+i*STATE_DIM] = -2*np.pi
            space_high = np.ones(STATE_DIM * STACK_STATE_FRAMES, ) * (BOUND_OF_SPACE)
        for i in xrange(0,STACK_STATE_FRAMES):
            space_high[1+i*STATE_DIM] = 2*np.pi
        return Box(low=space_low, high=space_high)
    ...
    def reset(self):
        ...
        if FN == '/home/ryan/data/navi/socially compliant/':
            self.FPS = 0.1
            ...
            if FN == '/home/ryan/data/navi/ewap_dataset_full/seq_eth/' or \
             FN == '/home/ryan/data/navi/ewap_dataset_full/seq_hotel/':
                self.FPS = 0.4
                minTrajSize = 10
            else:
                self.FPS = 0.1
    ...
    def step(self, action):
        ...
        if done:
            ...
            # reward = 500 * (R - self.lastR)
            # self.lastR = R
            # reward = -10 * abs(action[1]/MAX_VR)
            VR = action[1]
            if VR < MIN_VR:
                VR = 0
    ...
    def show(self):
        ...
        plt.scatter(self.trajectoryX[self._t-1], self.trajectoryY[self._t-1], color='green',marker='v',s=20)
        plt.scatter(self.obsCor[:, 0], self.obsCor[:, 1],color = 'green',s=2)
        plt.scatter(self.des[0],self.des[1], color = 'red')
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.savefig('/home/ryan/DL/navi/results.png')
        if self.DistToGoal < REACH_TARGET_BOUND:
            plt.savefig('/home/ryan/DL/navi/results_suc.png')
        else:
            plt.savefig('/home/ryan/DL/navi/results_fail.png')
        plt.clf()
    ...
    def continuouRW(self,action):
        obsF = self.trueState[4 + 2 * NUMBER_OF_PED + 2 * NUMBER_OF_COM]
        reward = -5*self.DistToGoal - abs(self.dist2Com-DESIRED_COM_DISTANCE) + min(obsF,2)
        if self.minPedDist <=4:
            reward -= np.cos(self.minPedRho)
        else:
            reward += 1
        return reward