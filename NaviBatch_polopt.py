...
class BatchSampler(Sampler):
    ...
    def process_samples(self, itr, paths):
        ...
        if not self.algo.policy.recurrent:
            ...
            states = tensor_utils.concat_tensor_list([path["states"] for path in paths])
        ...
        logger.record_tabular('AverageVR', np.mean(vR))
        logger.record_tabular('SuccRate', float(success)/len(paths))

        return samples_data


class NaviBatchPolopt(RLAlgorithm):
    """
    ...
    def train(self):
        ...
        for itr in xrange(self.current_itr, self.n_itr):
            with logger.prefix('itr #%d | ' % itr):
                ...
                self.optimize_policy(itr, samples_data,self.n_itr)
